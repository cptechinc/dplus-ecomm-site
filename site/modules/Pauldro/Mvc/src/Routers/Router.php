...
class Router extends WireData {
	...
	public function route() {
		...
		if (array_key_exists(1, $this->routeInfo)) {
			$this->controllerClass = $this->routeInfo[1][0];
		}
	...
	protected function handleException(Exception $e) {
		...
		if ($config->twigloader->exists('error-pages/' . $e->getCode() . '.twig')) {
			return $config->twig->render('error-pages/' . $e->getCode() . '.twig', ['error' => $e]);
		}
		$error = new Exception('Oops! something went wrong, try again, or contact us if the problem persists', $e->getCode());
		return $config->twig->render('error-pages/generic.twig', ['error' => $error]);
	}
